...
async function salvarNotaSupabase(chave, valor) {
  ...
  if (data && data.length > 0) {
    ...
    if (error) console.error("Erro ao atualizar nota no Supabase:", error);
  } else {
    const { error } = await supabase
      .from("avaliacoes")
      .insert([registro]);

    if (error) console.error("Erro ao inserir nota no Supabase:", error);
  }
}
...
async function carregarNotasSupabase() {
  ...
  data.forEach(item => {
    const criterioIndex = CRITERIOS_GERAIS.indexOf(item.criterio);
    if (criterioIndex === -1) return;

    const chave = `${item.semana}_${item.turma}_${item.grupo}_C${criterioIndex}_${item.avaliadora}`;
    state[chave] = item.nota;
  });
